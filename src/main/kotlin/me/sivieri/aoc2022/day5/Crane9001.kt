...
class Crane9001(
    ...
    fun move(move: CraneMove) {
        val fromStack = getStack(move.from)
        val toStack = getStack(move.to)
        val boxes = (1..move.n).fold(emptyList<Char>()) { acc, _ ->
            acc + fromStack.elements.pop()
        }.reversed()
        boxes.forEach { toStack.elements.push(it) }
    }
